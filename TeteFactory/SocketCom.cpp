...
#define RECV_TIMEOUT_BASE 100    // recv 100ms超时

CSocketCom::CSocketCom()
{
	m_SockClient = INVALID_SOCKET;
	m_iRevTimeO = RECV_TIMEOUT_BASE;   // 至少是RECV_TIMEOUT_BASE
...
int CSocketCom::Connect(const TString& IPAddress, unsigned short portNum)
{
	Close();
...
int CSocketCom::SendData(const char* sendbuf, int sendbuflen)
{
	if (INVALID_SOCKET == m_SockClient) {
...
int CSocketCom::RecvData(char* recvbuf, int recvbuflen, int nTimeOut)
{
	if (INVALID_SOCKET == m_SockClient) {
		PrintErrorMsg(_T("socket未连接!"));
		return -1;
	...
	while(iTimes > 0){
		...
		CMarkPlatform::Instance()->Delay(10);  // 延时，避免界面卡死
	} 

	return 0;
}

bool CSocketCom::SendCmd(const char* sendbuf, int sendbuflen
	...
	, int nTimeOut)
{
	...
	for (int i = 0; i < iTimes; i++)
	{
		...
		CMarkPlatform::Instance()->Delay(10);  // 延时5ms，避免界面卡死
	}
	PrintErrorMsg(_T("未找到关键字！"));
	return false;
}

void CSocketCom::PrintErrorMsg(const TCHAR* sErr)
{
	m_sErr = sErr;
}