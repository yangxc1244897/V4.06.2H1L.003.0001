...
#define MUST_EX_STYLE		(LVS_EX_FULLROWSELECT | LVS_EX_SUBITEMIMAGES | LVS_EX_GRIDLINES)
...
void CReportCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) 
{
    ...
    else if (lplvcd->nmcd.dwDrawStage == (CDDS_ITEMPREPAINT | CDDS_SUBITEM))
    {
		...
		*pResult = CDRF_DODEFAULT;
		...
		{
			::SelectObject(lplvcd->nmcd.hdc, m_ftOld.m_hObject);
		}
		//add by liuxiao for HyperLink 2007.11.30 end
	}
#endif
...
BOOL CReportCtrl::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo) 
{
	...
	for(int i =0 ;i < m_pControlPtrArray->GetSize() ;i++)
	{
		...
		if(pSubItemInfo->m_ControlTypeInfo.m_pWnd)
		{
			if(pSubItemInfo->m_ControlTypeInfo.m_pWnd->GetDlgCtrlID() == (int) nID)
			{
				for(int ii =0 ;ii < pSubItemInfo->m_arrControlMessageInfo.GetSize() ;ii++)
				{
					...
					{
						ControlTypeInfo* p_ControlTypeInfo = &pSubItemInfo->m_ControlTypeInfo;
						p_ControlTypeInfo->m_pParent = GetParent();
						MessageInfo.pEventHandler(p_ControlTypeInfo);   //pSubItemInfo->m_pWnd
						break;
					}
				}
			}
...
void CReportCtrl::SetSortSeparator(LPCTSTR lpSortSeparator)
{
	...
	if (lpSortSeparator != NULL)
		m_pszSeparator = _tcsdup(lpSortSeparator);   //将lpSortSeparator copy to m_pszSeparator
}

int CReportCtrl::InsertItemEx(int nItem, LPCTSTR lpText, ...)  //可变参数
{
	EndEdit(TRUE);
	_UnsetSortedColumn();
	...
	if (IDX < 0)
 	...
 	va_list list;   //指向参数的指针 
	va_start(list, lpText);    //初始化变量list,第二个参数是第一个可变参数的前一个参数，固定参数，这里指lptext

	for(int iColumn = 1; iColumn < GetColumnCount(); iColumn++)
	{
		LPCTSTR lp = va_arg(list, LPCTSTR);    //返回可变参数
		if (lp != NULL)
		{
...
void CReportCtrl::PreSubclassWindow() 
{
	...
	SetExtendedStyle(0);
	ASSERT(GetHeaderCtrl() != NULL);  //判断list控件
	
	// Create URL font
	CFont* pFont = GetFont();   //得到目前使用的字体
	if (!pFont)
	{
		//返回常用笔、画刷、字体、调色板的句柄
	...
	LOGFONT lf;
	pFont->GetLogFont(&lf); 
	m_ftOld.CreateFontIndirect((LOGFONT*)&lf); 
	//lf.lfUnderline = (BYTE) TRUE;
// 	lf.lfHeight = 18;
// 	lf.lfWidth = 8;
	m_ftURL.CreateFontIndirect((LOGFONT*)&lf);   //创建一个指定的字体
...
void CReportCtrl::_MouseClkMonitor(UINT nMsg, UINT nFlags, CPoint point, BOOL bTriggerEdit)
{
	LVHITTESTINFO hti;
	hti.pt = point;
	const int IDX = SubItemHitTest(&hti);   // row = IDX, hti.hti.iSubItem = col
	const BOOL BEFORE = CListCtrl::GetCheck(IDX) > 0;   //是否选中
	EndEdit(TRUE);
	...
	if (bTriggerEdit  && !STATSCHANGED && WASACTIVE)
		StartEdit(IDX, hti.iSubItem);

	if(_IsURL(IDX, hti.iSubItem) && _PtInText(point, IDX, hti.iSubItem) )
	{
		CItemData* p = (CItemData*)(CListCtrl::GetItemData(IDX));
		ShellExecute(NULL, _T("open"), p->arStrUrl[hti.iSubItem], NULL,NULL, SW_SHOW);
	}
}
...
void CReportCtrl::SetHeight(int nHeight)
{
	CImageList imageList;
	imageList.Create( nHeight, nHeight, ILC_COLOR4, 10, 10 ); 
	CListCtrl::SetImageList( &imageList, LVSIL_SMALL ); 
}

BOOL CReportCtrl::StartEdit(int nItem, int nSubItem)
{
	// Get the grid width and height
	if (!IsCellEditable(nItem, nSubItem)|| !_IsValidIndex(nItem) || nSubItem < 0 || nSubItem >= GetColumnCount())
		return FALSE;
	...
	if (FMT != m_dwPrevEditFmt)
	{
		...
		if (!m_pWndEdit->Create(ES_AUTOHSCROLL | ES_NOHIDESEL | ES_READONLY | WS_CHILD | WS_BORDER | FMT, CRect(0, 0, 1, 1), this, 0))
			return FALSE;
	}
	else
	{
		if (m_pWndEdit->GetSafeHwnd() == NULL
			&& !m_pWndEdit->Create(ES_AUTOHSCROLL | ES_NOHIDESEL | ES_READONLY | WS_CHILD | WS_BORDER | FMT, CRect(0, 0, 1, 1), this, 0))
		{
			return FALSE;
		}
	}
	...
	if (m_ptEditting.y > 0 && GetImageList() != NULL && GetItemImage(m_ptEditting.x, m_ptEditting.y) >= 0)
		rcEdit.DeflateRect(16, 0, 0, 0);   //放大或缩小一个rectangle,如果有图标，则rectangle的left边往右缩16个单位
...
void CReportCtrl::_AllocItemMemory(int nItem)
{
	...
	pData->arIsHyperLink.SetSize(COLS);
	pData->arIsHoverOn.SetSize(COLS);
	pData->arStrUrl.SetSize(COLS);
	pData->arStrTooltip.SetSize(COLS);
	pData->arIsEditable.SetSize(COLS);

	for (int i = 0; i < COLS; i++)
	{
		pData->aTextColors[i] = ::GetSysColor(COLOR_WINDOWTEXT);
		pData->aBkColors[i] = ::GetSysColor(COLOR_WINDOW);
		pData->arIsHyperLink[i] = FALSE;
		pData->arIsEditable[i] = FALSE;
	}
...
CRect CReportCtrl::_GetTextRect(int nItem, int nSubItem)
{
	CRect rect;
	ASSERT(nItem != -1);
	CDC* pDC = GetDC();
...
BOOL CReportCtrl::_PtInText(CPoint pt, int nItem, int nSubItem)
{
	return _GetTextRect(nItem, nSubItem).PtInRect(pt);
}
...
void CReportCtrl::_RedrawSubItem(int nItem, int nSubItem, BOOL HoverOn)
{
	CRect rect;
	CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
	p->arIsHoverOn[nSubItem] = HoverOn;
	GetSubItemRect(nItem, nSubItem, LVIR_LABEL, rect);
	m_toolTip.AddTool(this,p->arStrTooltip[nSubItem],_GetTextRect(nItem,nSubItem), 1);
	RedrawWindow(&rect);
}
...
void CReportCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if(nItem!=-1&&_IsURL(NowHti.iItem, NowHti.iSubItem)&&_PtInText(point, NowHti.iItem, NowHti.iSubItem))
		NowIn = TRUE;
	else
		NowIn = FALSE;
	...
	{
		if (_IsURL(PreHti.iItem, PreHti.iSubItem)&&PreIn)
			_RedrawSubItem(PreHti.iItem, PreHti.iSubItem, FALSE);
		if (_IsURL(NowHti.iItem, NowHti.iSubItem)&&NowIn)
			_RedrawSubItem(NowHti.iItem, NowHti.iSubItem, TRUE);
	}
	if (PreIn != NowIn)
	{
		PreIn?_RedrawSubItem(PreHti.iItem, PreHti.iSubItem, FALSE)
			:_RedrawSubItem(NowHti.iItem, NowHti.iSubItem, TRUE);
	}
...
BOOL CReportCtrl::_IsURL(int nItem, int nSubItem)
{
	if ( nItem < GetItemCount() && nItem >= 0 
		&& nSubItem < GetColumnCount() && nSubItem >= 0)
	{
		CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
		return (p->arIsHyperLink[nSubItem]);
	}else
		return FALSE;
}

BOOL CReportCtrl::SetItemURL(int nItem, int nSubItem, CString strURL)
{
	if ( nItem < GetItemCount() && nItem >= 0 
		&& nSubItem < GetColumnCount() && nSubItem >= 0)
	{
		CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
		p->arIsHyperLink[nSubItem] = TRUE;
		p->arStrUrl[nSubItem] = strURL;
		return TRUE;
	}else
		return FALSE;
}
...
////add by popo  2007.12.1 begin
void CReportCtrl::SetCellEditable(int nItem, int nColumn, BOOL IsEditable)
{
	if ( nItem < GetItemCount() && nItem >= 0 
		&& nColumn < GetColumnCount() && nColumn >= 0) 
	{
		CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
		p->arIsEditable[nColumn] = IsEditable;
	}	
}

void CReportCtrl::SetItemEditable(int nItem, BOOL IsEditable)
{
	CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
	for (int nColumn = 0; nColumn < GetColumnCount(); nColumn++)
	{
		SetCellEditable(nItem, nColumn, IsEditable);
	}
}

void CReportCtrl::SetColumnEditable(int nColumn, BOOL IsEditable)
{
...
void CReportCtrl::SetEditable(BOOL IsEditable)
{
	if (!IsEditable)
		EndEdit(TRUE);
	for (int nColumn = 0; nColumn < GetColumnCount(); nColumn++)
	{
		SetColumnEditable(nColumn, IsEditable);
	}
}

BOOL CReportCtrl::IsCurrentCellEditable() const
{
	int nItem = m_ptEditting.x;
	int nColumn = m_ptEditting.y;
	if (nItem >= 0 && nItem < CListCtrl::GetItemCount()
		&& nColumn>= 0 && nColumn < GetColumnCount() )
	{
		CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
		return p->arIsEditable[nColumn];
	}
	else return FALSE;
}

BOOL CReportCtrl::IsCellEditable(int nItem, int nSubItem) const
{
	if (	nItem >= 0 && nItem < CListCtrl::GetItemCount()
		&& nSubItem>= 0 && nSubItem < GetColumnCount() )
	{
		CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
		return p->arIsEditable[nSubItem];
	}
	else return FALSE;
}

BOOL CReportCtrl::SetItemURL(int nItem, int nSubItem, CString strURL, CString strTooltip)
{
	if ( nItem < GetItemCount() && nItem >= 0 
		&& nSubItem < GetColumnCount() && nSubItem >= 0)
	{
		CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
		p->arIsHyperLink[nSubItem] = FALSE;
		p->arStrUrl[nSubItem] = strURL;
		p->arStrTooltip[nSubItem] = strTooltip;
		return TRUE;
	}else
		return FALSE;
}

CString  CReportCtrl::GetItemURL(int nItem, int nSubItem)
{
	CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
	return p->arStrUrl[nSubItem];
}
...
BOOL CReportCtrl::SetItemControl(enCONTROLTYPE enControlType,enMETHONTYPE enMethonType,int nItem, int nSubItem,LPCTSTR lpControlName/*=NULL*/)
{
	//插入控件
	SubItemControlsInfo * pSubItemInfo = new SubItemControlsInfo();
	pSubItemInfo->m_ControlTypeInfo.m_enControlType = enControlType;
	pSubItemInfo->m_ControlTypeInfo.m_enMethonType = enMethonType;
	pSubItemInfo->m_ControlTypeInfo.m_nItem = nItem;
	pSubItemInfo->m_ControlTypeInfo.m_nSubItem = nSubItem;
	pSubItemInfo->m_ControlTypeInfo.m_pList = this;

	CFont * pFont = GetFont();
	DWORD dwStyle = WS_CHILD | WS_TABSTOP;    // WS_TABSTOP获取tab键的响应WS_BORDER|
	CRect ControlSize (10, 10, 100, 100);

	switch(enControlType)
	{
	case EDITBOX:
		pSubItemInfo->m_ControlTypeInfo.m_pWnd = new CEdit();
		((CEdit*)pSubItemInfo->m_ControlTypeInfo.m_pWnd)->Create (dwStyle | WS_BORDER,ControlSize,this ,m_iControlIDCounter);	
		break;
	case CHECKBOX:
		pSubItemInfo->m_ControlTypeInfo.m_pWnd = new CButton();
		((CButton*)pSubItemInfo->m_ControlTypeInfo.m_pWnd)->Create (lpControlName,dwStyle|BS_AUTOCHECKBOX,ControlSize,this ,m_iControlIDCounter);	
		break;
	case COMBOBOX:
		pSubItemInfo->m_ControlTypeInfo.m_pWnd = new CComboBox();
		((CComboBox*)pSubItemInfo->m_ControlTypeInfo.m_pWnd)->Create(dwStyle|WS_VSCROLL|CBS_DROPDOWNLIST,ControlSize, this,m_iControlIDCounter);
		break;
	case LISTBOX:
		pSubItemInfo->m_ControlTypeInfo.m_pWnd = new CListBox();
		((CListBox*)pSubItemInfo->m_ControlTypeInfo.m_pWnd)->Create (WS_VSCROLL|dwStyle,ControlSize,this ,m_iControlIDCounter);
		break;
	case BUTTON:
		pSubItemInfo->m_ControlTypeInfo.m_pWnd = new CButton();
		((CButton*)pSubItemInfo->m_ControlTypeInfo.m_pWnd)->Create (lpControlName,dwStyle,ControlSize,this ,m_iControlIDCounter);
		break;
	}
	pSubItemInfo->m_ControlTypeInfo.m_pWnd->SetFont(pFont);
	int iDupIndex = _ControlIndexDuplicating(nItem, nSubItem);    
	if(iDupIndex > -1)
	{
		_DetachControl(iDupIndex);
	}
	m_pControlPtrArray->Add(pSubItemInfo);
	if(enControlType != CNONE)
	{
		m_iControlIDCounter+=1;
	}
	return TRUE;
}

void CReportCtrl::MapControlsForSubItems()
{
	//改变插入控件的大小
	CRect CtrlRect;
	CRect iconRect;
	...
	for(int i =0 ;i <m_pControlPtrArray->GetSize() ;i++)
	{
		...
		int nItem = pSubItemControlsInfo->m_ControlTypeInfo.m_nItem;
		int nSubItem = pSubItemControlsInfo->m_ControlTypeInfo.m_nSubItem;
		if(pSubItemControlsInfo)
		{
			GetSubItemRect(nItem, nSubItem, LVIR_LABEL,CtrlRect);
			GetSubItemRect(nItem, nSubItem,LVIR_ICON,iconRect);
			CtrlRect.bottom+=3;
			if(nSubItem == 0)
			{
				CtrlRect.left+=iconRect.right;
			}
			if (BUTTON == pSubItemControlsInfo->m_ControlTypeInfo.m_enControlType)
			{
				CtrlRect.DeflateRect(10,2,10,4);
			}
			else if (EDITBOX == pSubItemControlsInfo->m_ControlTypeInfo.m_enControlType)
			{
				CtrlRect.DeflateRect(5, 2, 5, 4);
			}
			if(pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd)
				pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd->MoveWindow(&CtrlRect);

			strData = GetItemText(nItem,nSubItem);		

			switch(pSubItemControlsInfo->m_ControlTypeInfo.m_enControlType)
			{
			case CHECKBOX:
				break;		
			case EDITBOX:
				pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd->SetWindowText(strData);
				break;
			case COMBOBOX:
				((CComboBox*)pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd)->SelectString(-1,strData);
				break;
			case LISTBOX:
				((CListBox*)pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd)->SelectString(-1,strData);
				CtrlRect.bottom += (((CListBox*)pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd)->GetCount ()-1)*2;
				((CListBox*)pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd)->GetItemHeight(0);
				pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd->MoveWindow(&CtrlRect);
				break;
			}
		}
...
void CReportCtrl::AttachEvent_HandlerForItemControl(int nItem, int nSubItem, UINT unMessage, void (__cdecl *pEventHandler)(ControlTypeInfo* pControlTypeInfo))
{
	SubItemControlsInfo * pSubItemInfo;
	for(int i =0 ;i < m_pControlPtrArray->GetSize() ;i++)
	{
		pSubItemInfo = (SubItemControlsInfo *) m_pControlPtrArray->GetAt(i);
		if(pSubItemInfo->m_ControlTypeInfo.m_nItem == nItem && pSubItemInfo->m_ControlTypeInfo.m_nSubItem == nSubItem)
		{	
			ControlMessageInfo MessageInfo;
...
ControlTypeInfo* CReportCtrl::GetItemControlInfo(int nItem, int nSubItem)
{
	SubItemControlsInfo * pSubItemInfo;
	for (int i = 0; i < m_pControlPtrArray->GetSize(); i++)
	{
		...
		if (pSubItemInfo->m_ControlTypeInfo.m_nItem == nItem && pSubItemInfo->m_ControlTypeInfo.m_nSubItem == nSubItem)
		{
			return &pSubItemInfo->m_ControlTypeInfo;
			break;
		}
	}
	return nullptr;
...
void CReportCtrl::EnableControls(BOOL bEnable)
{
	SubItemControlsInfo * pSubItemControlsInfo;
	...
	for(int i =0 ;i <m_pControlPtrArray->GetSize() ;i++)
	{
		...
		{
			if(pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd)
				pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd->EnableWindow(bEnable);
		}
	}
}

void CReportCtrl::ShowControls(BOOL bShow)
{
	SubItemControlsInfo * pSubItemControlsInfo;
	...
	for(int i =0 ;i <m_pControlPtrArray->GetSize() ;i++)
	{
		...
		{
			if(pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd)
				pSubItemControlsInfo->m_ControlTypeInfo.m_pWnd->ShowWindow(bShow);
		}
	}
}

int CReportCtrl::_ControlIndexDuplicating(int nItem, int nSubItem)
{
	//检查对应位置是否存在控件
	...
	for(int i =0 ;i <m_pControlPtrArray->GetSize() ;i++)
	{
		...
		if(pInfo->m_ControlTypeInfo.m_nItem == nItem && pInfo->m_ControlTypeInfo.m_nSubItem == nSubItem)
		{
			return i;
		}
	}
...
BOOL CReportCtrl::_DetachControl(int nDupIndex)
{
	//删除控件
	SubItemControlsInfo * pInfo;
	pInfo = (SubItemControlsInfo *) m_pControlPtrArray->GetAt(nDupIndex);
	if(pInfo)
	{
		if(pInfo->m_ControlTypeInfo.m_pWnd)
		{
			pInfo->m_ControlTypeInfo.m_pWnd->DestroyWindow();
			pInfo->m_ControlTypeInfo.m_pWnd->m_hWnd = NULL;
			delete pInfo->m_ControlTypeInfo.m_pWnd;
			pInfo->m_ControlTypeInfo.m_pWnd = NULL;
		}
		pInfo->m_ControlTypeInfo.m_pList = NULL;
		delete pInfo;
		m_pControlPtrArray->RemoveAt(nDupIndex);
		return TRUE;
	}
	return FALSE;
}
void CReportCtrl::ReleaseControls()
{
	//删除所有控件
	SubItemControlsInfo * pInfo;
	for(int i =0 ;i <m_pControlPtrArray->GetSize() ;i++)
...
void CReportCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	for(int i = iTopMostItemIndex ; i < (iTopMostItemIndex + GetCountPerPage() ) ; i++)
	{
	//	if(GetFirstItem(RC_ITEM_SELECTED,-1) == i)
		{
			bShowItem = TRUE;
			break;
		}
