...
/********** NSocketServerFins *******************************************************************/
NSocketServerFins::NSocketServerFins()
{
    m_IsLink = false;
    m_IsClose = false;
    mpCallBack = NULL;
...
void NSocketServerFins::Register(void *hWin, int id, CBFun pCallBack)
{
    mhWin = hWin;
    mId = id;
    mpCallBack = pCallBack;
}
bool NSocketServerFins::Init(int port)
{
    m_IsLink = false;
    WSADATA wsd;
    bool rVal = true;
    ...
    do {
		...
		AfxBeginThread(&AcceptThdSFins, this);
    } while (0);

    return rVal;
}
bool NSocketServerFins::Send(CString msg)
{
...
bool NSocketServerFins::Accept()
{
    ...
    if (SOCKET_ERROR == recv(mSocketLink, mRecvBuf, BUF_SIZE, 0))
	{
		...
		{
			mpCallBack(mhWin, mId, "C Link OK",7);
			m_IsLink = true;
			m_IsClose = false;
			AfxBeginThread(&RecvThdSFins, this);
		}
	}
	else
	{
		...
		m_IsLink = true;
		m_IsClose = false;
        mpCallBack(mhWin, mId, mRecvBuf, BUF_SIZE);
		AfxBeginThread(&RecvThdSFins, this);
	}
...
bool NSocketServerFins::Recv()
{
    ...
    {
		mpCallBack(mhWin, mId, "Link ERROR",10);
...
bool NSocketServerFins::IsLink()
{
	return m_IsLink;
}
/********** NSocketClientFins *******************************************************************/
NSocketClientFins::NSocketClientFins()
{
    m_IsLink = false;
    m_IsClose = false;
    mpCallBack = NULL;
...
void NSocketClientFins::Register(void *hWin, int id, CBFun pCallBack)
{
    mhWin = hWin;
    mId = id;
    mpCallBack = pCallBack;
} 
bool NSocketClientFins::Open(const char *strAddr, int port)
{
    if (m_IsLink)
    {
        return true;
    ...
    m_IsLink = true;
    m_IsClose = false;
    mpthreadRecv = AfxBeginThread(&RecvThdCFins, this);

    return true;
}
bool NSocketClientFins::Send(const char* pData, int nLength)
{
	//if (!IsLink() || m_IsClose) {
	//	return false;
	//}
	...
	while (!m_IsClose && i-- > 0) {
		ProcessMessage();
		int retVal = send(mSocketClient, pData, nLength, 0);
		if (retVal == SOCKET_ERROR) {
			int err = WSAGetLastError();
...
bool NSocketClientFins::Send(CString msg)
{
    if (!IsLink() || m_IsClose)
    {
        return false;
...
bool NSocketClientFins::Recv()
{
	...
	do {
	    ...
	    ZeroMemory(tempBuf, BUF_SIZE);
        int retVal = recv(mSocketClient, tempBuf /*(char*)mRecvBuf*/, BUF_SIZE, 0);
        if (SOCKET_ERROR == retVal)
        {
            int err = WSAGetLastError();
            if (err == WSAEWOULDBLOCK)
        ...
        else if (NULL != mpCallBack)
        {
           /* mpCallBack(mhWin, mId, mRecvBuf);
			Sleep(50);
            break;*/
			mpCallBack(mhWin, mId, tempBuf, retVal);
        }
	}while(0);
...
bool NSocketClientFins::IsLink()
{
	return m_IsLink;
}
void NSocketClientFins::Close()
{
    m_IsClose = true;
	Sleep(200);
    if (m_IsLink)
...
{
	NSocketServerFins *socketS = (NSocketServerFins *)p;
	while (1)
	{
		ProcessMessage();
		if (!socketS->Accept())
...
{
    NSocketClientFins *socketC = (NSocketClientFins *)p;
    while (1)
    {
        if (!socketC->Recv())
        {
